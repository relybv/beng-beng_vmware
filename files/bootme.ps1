...
# This script installs puppet 4.x and deploy the manifest using puppet apply -e "include beng_vmware"
# Usage:
# <powershell>
# Set-ExecutionPolicy Unrestricted -Force
# icm $executioncontext.InvokeCommand.NewScriptBlock((New-Object Net.WebClient).DownloadString('https://raw.githubusercontent.com/relybv/dirict-beng_vmware/master/files/bootme.ps1')) -ArgumentList ("beng_vmware")
#</powershell>


  param(
    [string]$role = "beng_vmware"
  )

  $puppet_source = "https://github.com/relybv/dirict-beng_vmware.git"
  ...
  if(!(Test-Path -Path "C:\ProgramData\PuppetLabs\code\modules\beng_vmware" )){
    $clone_args = @("clone",$puppet_source,"C:\ProgramData\PuppetLabs\code\modules\beng_vmware" )
  } else {
    $clone_args = @("pull")
    Set-Location -Path "C:\ProgramData\PuppetLabs\code\modules\beng_vmware"
  }
